...
def load_world(directory, screen, width, height, INVENTORY_HEIGHT, BLOCK_WIDTH):
    ...
    with open(f"{directory}/inventory.json", "r") as inventory_file:
        ...
        inventory = Inventory.fill_from_dict(inventory_dict, screen, height, width, INVENTORY_HEIGHT)
...
def generate_world(screen):
    ...
    inventory = Inventory(screen, screen_height_px, screen_width_px, INVENTORY_HEIGHT)

    # initialize player in grid
    player = Player(grid, screen, ((grid_width * BLOCK_WIDTH) // 2), 0, BLOCK_WIDTH)
...
# create the screen
screen_width_px, screen_height_px = 1000, 600
...
while running and menu.menu_running :
    ...
    if menu.run_game:
        ...
        if keys[pygame.K_a]:
            dx += player.player_speed * -1
        if keys[pygame.K_d]:
            dx += player.player_speed 
        keys = pygame.key.get_pressed()
        if keys[pygame.K_w] or keys[pygame.K_SPACE]:
            if not player.is_not_block_below():
        ...
        if build_mode and held_time > BUILD_HOLD_THRESHOLD:
            ...
            if held_time > DESTROY_HOLD_THRESHOLD:
                inventory.add_item(type(grid.get(affected_x, affected_y)))
                grid.set(affected_x, affected_y, None)


        # apply gravity and jumping
        if player.y_vel + Y_ACCELERATION < BLOCK_WIDTH: #limits gravity at 1 block per tick
            dy += player.y_vel + Y_ACCELERATION


        # check if motion is legal
        player.is_move_ok_y(floor(dy))
        # if player.is_move_ok_y(floor(dy)): player.y += dy
    ...
    # run menu
        ...
        if menu.run_game: # check for if it's time to move on and generate world before launch
            if menu.load_world:
                ...
                grid, inventory, player = generate_world(screen)
                camera_x = player.x + (player.x_size // 2) - (screen_width_px // 2)
                camera_y = player.y + (player.y_size // 2) - (screen_height_px // 2)
                cur_camera_y = camera_y
                game_is_loaded = True
