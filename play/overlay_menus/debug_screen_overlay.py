...
class Debug_Overlay:
    def __init__(self, window, grid, player):
        self.window = window
        self.grid = grid
        self.player = player
    ...
    def rerender(self, margin, row_width):
        self.font_height = row_width // 3
        _f = font_manager.get()
        self.font = pygame.font.Font(str(_f.PixeloidMono), self.font_height)
        self.margin_left = margin

        allow_start_y = self.player.get_health_bar_height()
        if allow_start_y > 0:
            self.margin_top = allow_start_y + 6
        else:
            self.margin_top = self.player.health_bar.margin
    ...
    def draw(self):
        ...
        self.window.blit(surface, (self.margin_left, self.get_line_start_y(1)))

        text = f'Region ID: {Region.get_region_id_from_global_x(x)}'
        surface = self.font.render(text, True, (255, 255, 255))
        self.window.blit(surface, (self.margin_left, self.get_line_start_y(2)))

        text = f'Chunk ID: {self.cur_chunk}'
        surface = self.font.render(text, True, (255, 255, 255))
        self.window.blit(surface, (self.margin_left, self.get_line_start_y(3)))

        text = f'FPS: {self.fps}'
        surface = self.font.render(text, True, (255, 255, 255))
        self.window.blit(surface, (self.margin_left, self.get_line_start_y(4)))
