...
class Spruce_Log(Block):
    ...
    def draw_manual(screen, x, y, block_width, being_mined=False, is_grid_coordinates=True, use_alt_drawing=False):
        ...
        primary_color = (74, 68, 65)
        secondary_color = (40, 35, 33)
...
class Mahogany_Log(Block):
    ...
    def draw_manual(screen, x, y, block_width, being_mined=False, is_grid_coordinates=True, use_alt_drawing=False):
        ...
        primary_color = (120, 95, 83)
        secondary_color = (89, 53, 18)
...
class Mahogany_Leaves(Block):
    str_name = "Mahogany Leaves"
    ticks_to_mine = 18
    sappling_drop_chance = 0.12
    ...
    def physics(self):
        if (self.anchor_x is not None and self.anchor_y is not None) and not isinstance(self.grid.get(self.anchor_x, self.anchor_y), Mahogany_Log):
            if self.ticks_till_physics > self.tick_threshold:
                self.onDestroy()
            else:
                self.ticks_till_physics += 1
...
class Spruce_Planks(Block):
    str_name = "Spruce Planks"
    ticks_to_mine = 38
    ...
    def draw_manual(screen, x, y, block_width, being_mined=False, is_grid_coordinates=True, use_alt_drawing=False):
        ...
        # --- palette (derived from spruce log: primary (82,72,62), secondary (46,38,30)) ---
        base = (105 + added,  95 + added,  88 + added)
        seam = ( 80 + added,  71 + added,  65 + added)   # darker separator
        grain = ( 93 + added,  84 + added,  77 + added)  # subtle grain line
        highlight = (118 + added, 107 + added,  99 + added)
...
class Spruce_Sappling(Block):
    ...
    def grow_tree(self):
        from world.world_creation.structures.structures import Col_Spruce_Tree
        struct_odds = random.random()
        tree = Col_Spruce_Tree(struct_odds, self.x-1, self.y+1, self.y+1, self.grid, self.grid, 0)
        for col_num in range(tree.get_width(struct_odds)):
            tree.set_fg_col_to_grid_directly(col_num, self.y+1, None)
    ...
    def draw_manual(screen, x, y, block_width, being_mined=False, is_grid_coordinates=True, use_alt_drawing=False):
        ...
        bark      = c((74, 68, 65))
        bark_dark = c((40, 35, 33))
        # Match the actual Spruce_Leaves block palette
        leaf_base = c((78, 108, 96))
        leaf_dark = c((68,  98, 87))
        leaf_hi   = c((87, 116, 104))
...
class Mahogany_Sappling(Block):

    str_name = "Mahogany Sapling"
    ticks_to_mine = 12
    pass_through = True
    draw_background = True
    ignore_shading_from = {
        ...
        if self.grid.in_bounds(self.x, self.y + 1):
            if self.grid.get(self.x, self.y + 1) is None: # this means that the block under is empty
                ...
                if not (self.grid.in_bounds(self.x-1, self.y-self.full_tree_height+1) and self.grid.in_bounds(self.x+1, self.y-self.full_tree_height+1)):
                    ...
                    return
                for y in range(self.y - self.full_tree_height + 1, self.y):
                    for x in range(self.x - 2, self.x + 4):
                        if self.grid.get(x, y) is not None:
                            self.ticks_till_physics = 0
                            return
