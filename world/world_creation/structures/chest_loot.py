...
class Chest_Loot:
    ...
    def get_slot(self, odds):
        rolling_odds = 0
        for loot_list_spot in self.loot_list:
            if loot_list_spot.odds_of_appearing + rolling_odds > odds:
                count = max(1, int(int(hashlib.sha256(f"{odds}_count".encode()).hexdigest(), 16) / (2**256 - 1) * loot_list_spot.max_count))
