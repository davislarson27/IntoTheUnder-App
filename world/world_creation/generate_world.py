...
class Grid_Superstructure:
    ...
    def generate_world(self):
        ...
        def generate_cave(start_x, start_y, ground_level, max_cave_depth, is_water_cave, saltpeter_chance):
            ...
            ceiling = start_y - 1
            floor = start_y + 1
            for x in range(start_x + 1, self.foreground_grid.width):
                ...
                if ceiling + ceiling_change < self.foreground_grid.height and ceiling + ceiling_change >= ground_level[x]:
                    ceiling += ceiling_change
                if floor + floor_change < self.foreground_grid.height and floor + floor_change >= ground_level[x]:
                    floor += floor_change
                ...
                for y in range(ceiling+1 , floor):
                    if is_water_cave:
                        self.foreground_grid.set(x, y, Water, True)
                    else:
                        self.foreground_grid.set(x, y, None)
                    create_cave = True
                ...
                if floor - ceiling - 1 > 1 and not is_water_cave: # requires that there be at least two spots open to generate
                    if random.random() < saltpeter_chance:
                        self.foreground_grid.set(x, ceiling+1, Saltpeter)

            # now dig out the original center block if the cave began to expand
            if create_cave: 
                if is_water_cave:
                    self.foreground_grid.set(start_x, start_y, Water, True)
                else:
                    self.foreground_grid.set(start_x, start_y, None)
        ...
        cur_biome_start = 0
        cur_biome_size = cur_biome.biome_base_size + floor(random.random() * self.foreground_grid.biome_size_variability)
        ...
        for x in range(self.foreground_grid.width):
            ...
            self.foreground_grid.set(x, cur_floor_level, cur_biome.surface_layer)
            ...
            for y in range(1, cur_dirt_depth):
                self.foreground_grid.set(x, cur_floor_level + y, cur_biome.sub_surface_layer)
            
            for y in range(cur_dirt_depth, self.foreground_grid.height):
                self.foreground_grid.set(x, cur_floor_level + y, cur_biome.deep_layer)

        # generate ores
        for x in range(2, self.foreground_grid.width - 2):
        ...
        for x in range(self.foreground_grid.width):
            cur_biome = cur_biome_by_x[x]
            for y in range(ground_level[x] + 1, self.foreground_grid.height):
        ...
        lake_level = self.foreground_grid.height #throws in a number that should always end a lake if it ever triggered
        fill_lake = False
        for x in range(2, self.foreground_grid.width - 2): # cuts off last 2 blocks on each end of the world
            if fill_lake: #if we are filling right now
                if ground_level[x] > lake_level:
                    for water_fill_y in range(lake_level, ground_level[x]):
                        self.foreground_grid.set(x, water_fill_y, Water, True)
                else:
                    fill_lake = False
            else:
                cur_biome = cur_biome_by_x[x]
                if cur_biome.lake_chance > 0 and ground_level[x] > MIN_LAKE_DEPTH:
                    ...
                    if ground_level[x-1] < ground_level[x]: #means a drop off started and the lake can begin forming
                        if random.random() < cur_biome.lake_chance:
                            ...
                            self.foreground_grid.set(x, lake_level, Water, True)
        ...
        for x in range(self.foreground_grid.width):
            ...
            if cur_biome.recipe_burrow_chance > 0:
                ...
                isValidBuild = True
                if x + Recipe_Burrow.get_width() < self.foreground_grid.width and y_start + Recipe_Burrow.get_depth() < self.foreground_grid.height and y_start + Recipe_Burrow.get_height() > 0: #check for if in bounds
                    ...
                    if isValidBuild and random.random() < cur_biome.recipe_burrow_chance:
                        ...
                        buildInstructions = Recipe_Burrow.getStructureInstructions(x, y_start, self.foreground_grid)
            ...
            if cur_biome.tree_chance > 0:
                if x + 2 < self.foreground_grid.width and x - 2 > 0: #check for if in bounds
                    if not has_above_ground_object[x - 1] and not has_above_ground_object[x] and not has_above_ground_object[x+1]:
                        if ground_level[x] == ground_level[x+1] or ground_level[x] == ground_level[x-1]:
                            if random.random() < cur_biome.tree_chance:
                                ...
                                if x + 2 < self.foreground_grid.width: has_above_ground_object[x+2] = True

            # cactus
            if cur_biome.cactus_chance > 0:
                if x + 1 < self.foreground_grid.width and x - 1 > 0: #check for if in bounds
                    if not has_above_ground_object[x - 1] and not has_above_ground_object[x] and not has_above_ground_object[x+1]:
                        if ground_level[x] == ground_level[x+1] or ground_level[x] == ground_level[x-1]:
            ...
            if cur_biome.small_bushes_chance > 0:
                if x + 1 < self.foreground_grid.width:
